...
static const int64_t nTargetTimespan = 60 * 60;  // darkfox 
unsigned int nTargetSpacing = 1 * 60; // darkfox - 1 minute
static const int64_t nInterval = nTargetTimespan / nTargetSpacing;

static const int64_t nDiffChangeTarget = 1;

unsigned int nStakeMinAge = 12 * 60 * 60; // darkfox - 12 hours
unsigned int nStakeMaxAge = -1; // darkfox - unlimited
unsigned int nModifierInterval = 10 * 60; // darkfox - time to elapse before new modifier is computed
...
const string strMessageMagic = "darkfox Signed Message:\n";
...
{
    int64_t nSubsidy = 95 * COIN;
    if (pindexBest->nHeight == 1)
    {
      nSubsidy = 75000 * COIN;
      return nSubsidy + nFees;
    }
    if (pindexBest->nHeight < 90)
    {
      nSubsidy = 1 * COIN;
      return nSubsidy + nFees;
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "darkfox", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
unsigned char pchMessageStart[4] = { 0xb1, 0xda, 0x3b, 0xcd };
